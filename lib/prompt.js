...
// ── Banner — always shows full ASCII art ─────────────────────────────────────
function banner(subtitle) {
    const B = (s) => _.cyan(_.bold(s));
    console.log("");
    console.log(B("  ██████╗  ██████╗  ██████╗ ██████╗ ██████╗ ███████╗"));
    console.log(B(" ██╔════╝ ██╔═══██╗██╔════╝██╔═══██╗██╔══██╗██╔════╝"));
    console.log(B(" ██║  ███╗██║   ██║██║     ██║   ██║██║  ██║█████╗  "));
    ...
    if (subtitle) console.log(_.dim(`  ${subtitle}`));
    console.log("");
}
...
async function cmdMenu() {
    banner('');
    p.intro(_.bold(_.bg_cyan(' ⚡ gocode — main menu ')));

    const action = orExit(await p.select({
        message: 'What do you want to do?',
        options: [
            { value: 'new',      label: _.cyan('⚡') + ' New Project',    hint: 'scaffold, push, deploy a new project' },
            { value: 'resume',   label: _.green('▶') + ' Resume',         hint: 'pick up where you left off' },
            { value: 'push',     label: _.cyan('↑') + ' Push',            hint: 'commit and push to GitHub' },
            { value: 'deploy',   label: '🚀 Deploy',                       hint: 'push to Netlify' },
            { value: 'list',     label: _.cyan('▦') + ' All Projects',    hint: 'see every project and status' },
            { value: 'status',   label: _.cyan('◉') + ' Status',          hint: 'detailed info on a project' },
            { value: 'edit',     label: _.yellow('✎') + ' Edit',          hint: 'rename · description · visibility' },
            { value: 'delete',   label: _.red('✕') + ' Delete',           hint: 'remove a project everywhere' },
            { value: 'note',     label: '📝 Quick Note',                   hint: 'add a note to Obsidian' },
            { value: 'settings', label: _.dim('⚙') + ' Settings',        hint: 'config · browser · doctor · backup' },
        ]
    }));

    if (action === 'settings') {
        const setting = orExit(await p.select({
            message: 'Settings',
            options: [
                { value: 'config',      label: '🔧 Setup Wizard',  hint: 'reconfigure gocode' },
                { value: 'set-browser', label: '🌐 Set Browser',   hint: 'choose default browser' },
                { value: 'doctor',      label: '🩺 Doctor',        hint: 'check all dependencies' },
                { value: 'backup',      label: '💾 Backup',        hint: 'back up registry and config' },
                { value: 'stats',       label: '📊 Stats',         hint: 'project statistics' },
                { value: 'update',      label: '⬆  Update gocode', hint: 'pull latest from GitHub' },
            ]
        }));
        p.outro(_.dim('running ' + setting + '...'));
        finish({ action: setting });
        return;
    }

    p.outro(_.dim('running ' + action + '...'));
    finish({ action });
}
